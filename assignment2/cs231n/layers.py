...
def affine_backward(dout, cache):
    ...
    db = np.squeeze(dout.T.dot(np.ones((N, 1))))
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, gamma, beta, sample_mean, sample_var, eps, num, denom)
        '''
        N, D = x.shape
	...
	#step4: calculate variance
        ...
        out = gammax + beta
        #store intermediate
        cache = (x,xhat,gamma,xmu,ivar,sqrtvar,var,mu,eps)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    x,xhat,gamma,xmu,ivar,sqrtvar,var,mu,eps = cache
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    sq = xmu ** 2
    #step4: calculate variance
    var = np.expand_dims(1./D * np.sum(sq, axis=1), axis=1)
    #step5: add eps for numerical stability, then sqrt
    sqrtvar = np.sqrt(var + eps)
    #step6: invert sqrtwar
    ivar = 1./sqrtvar
...
def layernorm_backward(dout, cache):
    ...
    dxhat = dgammax * gamma
    #step7
    divar = np.expand_dims(np.sum(dxhat*xmu, axis=1),axis=1)
    dxmu1 = dxhat * ivar
    #step6
    dsqrtvar = -1. /(sqrtvar**2) * divar
    #step5
    ...
    #step2
    dx1 = (dxmu1 + dxmu2)
    dmu = -1 * np.expand_dims(np.sum(dxmu1+dxmu2, axis=1), axis=1)
    #step1
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    F = b.shape[0]
    padded_H, padded_W = padded_x.shape[2:]
        ...
        for j in range(F):
            ...
            for k in range(0, padded_H - HH + 1, stride):
                ...
                for l in range(0, padded_W - WW + 1, stride):
                    out[i,j,Hp_i,Wp_i] = np.sum(padded_x[i,:,k:k+HH,l:l+WW] * w[j,:,:,:]) + b[j]
                    Wp_i += 1
                Hp_i += 1

#    out = np.sum(padded_x[list(range(N)),:,list(range(0, H, stride)):list(range(HH,HH+stride, stride)),list(range(0, W, stride)):list(range(WW,WW+stride, stride))]
#            * w[list(range(F)),:,:,:]+b, axis=())
    ...
    for i in range(N):
        for j in range(F):
            ...
            for k in range(0, padded_H - HH + 1, stride):
                Wp_i = 0
                for l in range(0, padded_W - WW + 1, stride):
                    dpadded_x[i,:,k:k+HH,l:l+WW] += w[j,:,:,:] * dout[i,j,Hp_i,Wp_i]
                    dw[j,:,:,:] += padded_x[i,:,k:k+HH,l:l+WW] * dout[i,j,Hp_i,Wp_i]
                    db[j] += dout[i,j,Hp_i,Wp_i]
                    Wp_i += 1
                Hp_i += 1
    dx = dpadded_x[:,:,pad:H+1,pad:W+1]
    ...
    for i in range(N):
        ...
        for j in range(0, H - pool_height + 1, stride):
            ...
            for k in range(0, W - pool_width + 1, stride):
                out[i,:,Hp_i,Wp_i] = np.amax(x[i,:,j:j+pool_height,k:k+pool_width], axis=((1,2)))
                Wp_i += 1
            Hp_i += 1
    ...
    for i in range(N):
        for c in range(C):
            ...
            for j in range(0, H - pool_height + 1, stride):
                Wp_i = 0
                for k in range(0, W - pool_width + 1, stride):
                    cutoff = np.amax(x[i,c,j:j+pool_height,k:k+pool_width])
                    dx[i,c,j:j+pool_height,k:k+pool_width] += (x[i,c,j:j+pool_height,k:k+pool_width] == cutoff) * dout[i,c,Hp_i,Wp_i]
                    Wp_i += 1
                Hp_i += 1
